  ...
  useEffect(() => {
    const fetchArticles = async () => {
      const { data } = await supabase
        .from("articles")
        .select("id, title, slug, excerpt, cover_image, published_at, created_at, views")
        .eq("status", "published")
    ...
    <section className="pt-32 pb-20">
      <div className="container">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <span className="font-mono text-xs text-primary tracking-widest uppercase">Publications</span>
          <h1 className="text-4xl md:text-5xl font-semibold text-foreground mt-3 mb-4">Blog & Actualités</h1>
          <p className="text-lg text-muted-foreground max-w-xl">Découvrez nos dernières publications sur la technologie et l'innovation au Burundi.</p>
        </motion.div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-sm text-muted-foreground mt-2">Revenez bientôt pour nos prochaines publications !</p>
          </div>
        ) : (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mt-12">
                ...
                <Link to={`/blog/${article.slug}`} className="group block rounded-xl border border-border bg-card overflow-hidden hover:border-primary/30 transition-all">
                  ...
                  <div className="p-6">
                    <div className="flex items-center gap-4 text-xs text-muted-foreground mb-3">
                      <span className="flex items-center gap-1"><Calendar size={12} /> {new Date(article.published_at || article.created_at).toLocaleDateString("fr-FR")}</span>
