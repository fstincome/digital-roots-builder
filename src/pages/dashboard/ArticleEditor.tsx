...
const ArticleEditor = () => {
  ...
  useEffect(() => {
    supabase.from("categories").select("*").eq("type", "article").order("name").then(({ data }) => setCategories(data || []));
    if (!isNew) {
      supabase.from("articles").select("*").eq("id", id).single().then(({ data }) => {
        if (data) {
          ...
          setStatus(data.status); setCategoryId(data.category_id || "");
        }
      });
    }
  }, [id]);
  ...
  const handleTitleChange = (val: string) => { setTitle(val); if (isNew) setSlug(generateSlug(val)); };
  ...
  const handleSave = async (publishStatus?: string) => {
    ...
    else { toast({ title: isNew ? "Article créé !" : "Article mis à jour !" }); navigate("/dashboard/articles"); }
    setLoading(false);
  };
    ...
    <div className="max-w-3xl">
      ...
      <div className="space-y-5">
        ...
        <div className="flex gap-3 pt-4">
          ...
          <Button variant="outline" onClick={() => handleSave("draft")} disabled={loading}>Sauvegarder comme brouillon</Button>
        </div>
      </div>
    </div>
  );
