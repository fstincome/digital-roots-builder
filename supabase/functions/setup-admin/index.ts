...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (existingAdmins && existingAdmins.length > 0) {
      return new Response(JSON.stringify({ error: "Un admin existe déjà" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    const { data: newUser, error: createError } = await supabaseAdmin.auth.admin.createUser({
      ...
      password,
      email_confirm: true,
      user_metadata: { full_name: "Admin SIGHT Africa" },
    });
